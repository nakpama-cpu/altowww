...
export default function Account() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <section className="bg-surface border border-border shadow-sm rounded-sm p-8 mb-6">
      ...
      <Dialog open={passwordOpen} onOpenChange={(o) => {
        ...
        <DialogContent className="max-w-[95vw] sm:max-w-md max-h-[90vh] overflow-y-auto p-4 sm:p-6 bg-surface border border-border shadow-sm rounded-sm">
          <DialogHeader>
...
function VerifyAddressDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] sm:max-w-lg md:max-w-2xl w-full p-3 sm:p-6 gap-2 sm:gap-3 max-h-[92vh] overflow-y-auto bg-surface border border-border shadow-sm rounded-sm">
        ...
        <form onSubmit={submit} className="space-y-2 sm:space-y-4 pt-1">
          <div className="rounded-lg bg-surface border border-border p-2.5 sm:p-5 space-y-1.5 sm:space-y-3">
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-lg md:max-w-2xl w-full p-3 sm:p-6 gap-2 sm:gap-3 max-h-[92vh] overflow-y-auto bg-surface border border-border shadow-sm rounded-sm">
        ...
        <form onSubmit={submit} className="space-y-2 sm:space-y-4 pt-1">
          ...
          {docType === "driving_licence" && !addressAlreadyVerified && (
            <div className="border border-amber-500/40 bg-amber-500/10 rounded-md p-2 sm:p-3 space-y-2 sm:space-y-3">
              ...
              {useForAddress && hasAddressOnFile && (
                <TextField dense label="Licence issue date" type="date" value={issuedOn} onChange={setIssuedOn} />
              )}
            </div>
          )}
...
function EditProfileDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] sm:max-w-lg w-full p-3 gap-2 max-h-[95vh] overflow-y-auto bg-surface border border-border shadow-sm rounded-sm">
        ...
        {tab === "phone" ? (
          ...
          <div className="space-y-2 sm:space-y-3 pt-1">
            ...
            <div className="rounded-lg bg-surface border border-border p-2.5 space-y-1.5">
              <div className="grid grid-cols-2 gap-2 items-end">
                <div>
                  ...
                  <select
                    value={addr.proof_of_address_type}
                    onChange={(e) => setAddr({ ...addr, proof_of_address_type: e.target.value as ProofOfAddressType })}
                    className="w-full h-8 bg-transparent border-b border-border py-0.5 font-body text-xs leading-none focus:outline-none focus:border-primary"
                  >
                    <option value="">Select…</option>
                    {ADDRESS_PROOF_TYPES.map((t) => (
                      <option key={t.value} value={t.value}>{t.label}</option>
              ...
              <div>
                <label className="block font-body text-[9px] sm:text-[10px] uppercase tracking-[0.15em] text-muted-foreground mb-0.5">Upload new proof of address</label>
                <input
                  type="file"
                  accept="application/pdf,image/jpeg,image/png,image/webp"
                  onChange={(e) => setFile(e.target.files?.[0] ?? null)}
